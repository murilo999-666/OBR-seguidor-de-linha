...
const int T_RETORNO = T_LATERAL - 250;   // ms — último lateral de retorno à linha
...
// ─────────────────────────────────────────────────────────────
//  Parâmetros de travessia de GAP e recuperação de linha
//  (gaps são sempre em linha RETA — regra OBR p.30; máx. 10 cm N1 / 15 cm N2)
// ─────────────────────────────────────────────────────────────
const uint8_t  VEL_GAP      = 170;  // PWM ao cruzar o gap / durante a busca
const uint8_t  VEL_GIRO     = 150;  // PWM dos giros de busca
const uint16_t T_CRUZAR_GAP = 900;  // ms indo RETO p/ atravessar o gap (ajuste no piso)
const uint16_t T_RE         = 250;  // ms de ré antes de cada nova tentativa
const uint16_t T_TENTA      = 350;  // ms avançando a cada tentativa de busca
const uint16_t T_GIRO_MIN   = 120;  // giro inicial "bem pouquinho" (ms)
const uint16_t T_GIRO_INC   =  80;  // crescimento do giro a cada tentativa (ms)
const uint16_t T_GIRO_MAX   = 600;  // giro máximo antes de reiniciar o leque (ms)

// Detecção de LINHA PERDIDA (gap) — CORREÇÃO DE POLARIDADE:
//   readLineBlack dá valor calibrado ALTO (~1000) SOBRE A LINHA e BAIXO (~0)
//   sobre o branco. Logo: sensor SOBRE a linha = valor >= LIMIAR_LINHA.
//   Linha perdida (gap) = TODOS os sensores ABAIXO de LIMIAR_LINHA (tudo branco).
const uint16_t LIMIAR_LINHA = 500;  // acima disso = sensor está sobre a linha preta

// Decisão GAP x CURVA pelo último erro antes de perder a linha:
//   |previousError| <  LIMIAR_CURVA → linha estava centrada  → era GAP   → vai RETO
//   |previousError| >= LIMIAR_CURVA → linha saiu pela ponta   → era CURVA → VIRA p/ o lado
//   (error vai de -2000 a +2000; sinal indica o lado: >0 = esquerda, <0 = direita)
const int      LIMIAR_CURVA = 1000; // limiar de erro p/ distinguir curva fechada de gap
const uint16_t T_GIRO_CURVA = 900;  // ms girando p/ o lado da curva antes de cair na busca

// Erro suavizado (média móvel): distingue gap de curva MUITO melhor que um único
// erro instantâneo. Numa curva o erro fica grande por vários ciclos → média cresce;
// num gap, mesmo com a oscilação do PID, a média fica perto de 0 → vai reto.
const float ALFA_ERRO = 0.25f;      // peso do erro novo na média (0..1; menor = mais suave)
const int   LIMIAR_CENTRADO = 400;  // |erro| abaixo disso = robô centrado → ZERA erroSuave
float erroSuave = 0;
...
// Travessia de gap / recuperação de linha
bool linhaVisivel();
bool aguardarLinha(uint16_t ms);
void recuperarLinha();
...
void robot_control()
{
  ...
  // LINHA PERDIDA (gap) = TODOS os sensores no BRANCO (valores baixos).
  // CORREÇÃO: a condição antiga ">= 980" exigia todos no PRETO (interseção) e por
  // isso NUNCA disparava no gap (onde é tudo branco). Resultado: no gap o
  // readLineBlack devolvia posição extrema p/ o último lado → o PID girava pro
  // último lado. Agora detectamos a perda corretamente: todos < LIMIAR_LINHA.
  bool perdida = true;
  for (uint8_t i = 0; i < SensorCount; i++)
    if (sensorValues[i] >= LIMIAR_LINHA) { perdida = false; break; }

  if (perdida)
  {
    digitalWrite(LED_BUILTIN, LOW);
    recuperarLinha();                             // cruza o gap; se falhar, busca até achar
    digitalWrite(PIN_LED_INT, LOW);               // apaga indicador de "procurando"
    position = qtr.readLineBlack(sensorValues);   // releitura após reencontrar a linha
    error = 2000 - position;
  }
  else
  {
    // Linha visível: atualiza a tendência do erro (p/ decidir gap x curva).
    // Se o robô voltou ao centro (erro perto de 0), ZERA a memória: não há curva
    // acontecendo agora, então um gap logo à frente é tratado como reta (vai reto).
    if (abs(error) < LIMIAR_CENTRADO)
      erroSuave = 0;
    else
      erroSuave = erroSuave * (1.0f - ALFA_ERRO) + error * ALFA_ERRO;
  }
...
// ═════════════════════════════════════════════════════════════
//  TRAVESSIA DE GAP + RECUPERAÇÃO DE LINHA
//  Gaps são sempre em linha RETA (regra OBR p.30): a estratégia é seguir
//  reto para atravessar a lacuna e reencontrar a linha do outro lado.
//  Indicação visual: PIN_LED_INT piscando = "procurando a linha".
// ═════════════════════════════════════════════════════════════

// true se ao menos um sensor está SOBRE a linha (valor calibrado ALTO).
// readLineBlack: sobre a linha ~1000; sobre o branco ~0. Portanto "sobre a
// linha" é valor >= LIMIAR_LINHA (e não "< 980", que era a polaridade invertida).
bool linhaVisivel() {
  qtr.readLineBlack(sensorValues);
  for (uint8_t i = 0; i < SensorCount; i++)
    if (sensorValues[i] >= LIMIAR_LINHA) return true;
  return false;
}

// Mantém o movimento já acionado pelo chamador por até 'ms', checando a linha
// o tempo todo. Para os motores ao sair. Retorna true assim que achar a linha.
bool aguardarLinha(uint16_t ms) {
  unsigned long t0 = millis();
  while (millis() - t0 < ms) {
    digitalWrite(PIN_LED_INT, (millis() / 100) % 2);   // pisca = procurando
    if (linhaVisivel()) {
      pararMotores();
      return true;
    }
...
// Recuperação de linha. Decide pela TENDÊNCIA do erro (erroSuave = média móvel
// do erro nos ciclos antes de perder a linha — mais confiável que um único erro):
//  FASE 1a — GAP  (erro pequeno → linha estava centrada): vai RETO p/ cruzar.
//  FASE 1b — CURVA (erro grande → linha saiu pela ponta):  VIRA p/ o lado da
//            curva (sinal do erro), em vez de ir reto.
//  FASE 2  — se a fase 1 não achar: busca progressiva — volta um pouco, gira
//            bem pouquinho e tenta de novo, alternando o lado e aumentando o
//            giro a cada tentativa (leque crescente), até reencontrar a linha.
void recuperarLinha() {
  int  errAnt      = (int)erroSuave;       // tendência do erro antes de perder a linha
  bool giraDireita = (errAnt < 0);         // err>0 = curva à esquerda; err<0 = à direita

  Serial.print(F("[PERDA] erroSuave="));
  Serial.print(errAnt);

  if (abs(errAnt) >= LIMIAR_CURVA) {
    // FASE 1b — CURVA FECHADA: gira para o lado da curva (NÃO vai reto)
    Serial.println(giraDireita ? F(" => CURVA: vira DIREITA") : F(" => CURVA: vira ESQUERDA"));
    if (giraDireita) {
      motor1.setSpeed(VEL_GIRO); motor1.backward();
      motor2.setSpeed(VEL_GIRO); motor2.backward();
    } else {
      motor1.setSpeed(VEL_GIRO); motor1.forward();
      motor2.setSpeed(VEL_GIRO); motor2.forward();
    }
    if (aguardarLinha(T_GIRO_CURVA)) { Serial.println(F("  -> achou (curva)")); return; }
  } else {
    // FASE 1a — GAP em linha reta: vai RETO para atravessar a lacuna
    Serial.println(F(" => GAP: vai RETO"));
    moverFrente(VEL_GAP);
    if (aguardarLinha(T_CRUZAR_GAP)) { Serial.println(F("  -> achou (gap reto)")); return; }
  }

  Serial.println(F("  fase 1 nao achou -> busca em leque"));

  // FASE 2 — busca progressiva até achar a linha (começa pelo lado do erro)
  uint16_t passoGiro = T_GIRO_MIN;
  while (true) {
    // volta um pouco (desfaz o avanço/giro que não encontrou a linha)
    moverRe(VEL_GAP);
    if (aguardarLinha(T_RE)) return;

    // gira "bem pouquinho" para um lado, checando a linha
    if (giraDireita) {
      motor1.setSpeed(VEL_GIRO); motor1.backward();
      motor2.setSpeed(VEL_GIRO); motor2.backward();
    } else {
    ...
    if (aguardarLinha(passoGiro)) return;

    // tenta avançar de novo
    moverFrente(VEL_GAP);
    if (aguardarLinha(T_TENTA)) return;

    // próxima tentativa: outro lado e giro um pouco maior (leque crescente)
    giraDireita = !giraDireita;
    passoGiro += T_GIRO_INC;
    if (passoGiro > T_GIRO_MAX) passoGiro = T_GIRO_MIN;
  }
}
